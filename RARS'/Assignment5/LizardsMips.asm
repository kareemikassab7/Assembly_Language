...
array : .space 1024
separation: .asciiz "  | "
SizeMessage: .asciiz "Please, Enter the number of rows or columns (they are equal): "
HMTrees: .asciiz "How many trees do you want to insert? "
RowInd: .asciiz "what is the row of the tree?"
ColInd: .asciiz "what is the column of the tree?"
NewLine: .asciiz "\n----------------------------------------\n"
space: .asciiz " "
.text
#ASSMPTIONS:
...
#t7 loads and holds the element while iterating to see its value and decide what to do
#t8 used in calculating indices (intitally has the adress of the slot we are in)
#t9 free
# s0 = 0
# s1 = 1 
# s2 = 2
...
syscall
add $t0, $v0, $zero # stores the input in $t0 to be used later
...
li $v0,5 #takes integer input from user
syscall
add $t3, $v0, $zero # stores the input in $t0 to be used later
###################################################################################################################################################
...
InputLoop:
li $v0, 4 # Code for Printing a string: The row message
la $a0, RowInd
syscall
li $v0,5 #takes integer input from user
syscall
add $t5, $v0, $zero # stores the input in $t0 to be used later
li $v0, 4 # Code for Printing a string
la $a0, ColInd
syscall
li $v0,5 #takes integer input from user
syscall
add $t6, $v0, $zero # stores the input in $t6 to be used later
...
mulu $t5, $t5, 4  # multiplies the number of slots by 4 to increment the size
...
SlotIteration:
lw $t7, ($t5) 	     # loads the number in the current slot to be compared
beq $t7, $zero, Free # if the number is zero, it is ok to place a lizard
beq $t7, $s2, Tree   # if the number is 2 then this is a tree
beq $t7, $s3, UnSafe # if the number is 3 then this is a Unsafe a.k.a under attack
Free:
...
sub $t1, $t1, 1
beq $t1, 0, NextColumn #loops over the whole size of the array
b SlotIteration
	
NextColumn: # This loops switches between the columns every n slots
add $t1, $t0, $zero # puts n in t1
add $t2, $t2, 4 # moves to the lement to the right of us
...
add $t8, $t5, $zero #address of  the slot we are traversing from
sub $a1, $t8, 0x10010000 # This is the adrss of the first slot in the array
div $a1, $a1, $t6        # we divide by length or width (same) * 4 to get the row index we are in
sub $a3, $t8, 0x10010000 # This is the difference between the adress we are on and the first one (total slots*4)
...
TraverseLeft:
sub $a2, $a2, 1 # the column moves to left while row constant
bge $a2, 0, TraverseLeftValid	# as long as were within width its ok
b TraverseLeftFinished 	#if not we are done traversing
TraverseLeftValid:
sub $t8, $t8, 4	# we move to the slot to the left
lw $s6, ($t8)	# we cload it
...
TraverseUp:
sub $a1, $a1, 1		# the row moves to up while column constant
bge $a1, 0, TraverseUpValid	# as long as were within height its ok
b TraverseUpFinished		#if not we are done traversing
TraverseUpValid:
sub $t8, $t8, $t6    #go to element above us
lw $s6, ($t8)
...
TraverseDown:
add $a1, $a1, 1		#increments row index while column is constant
blt $a1, $t0, TraverseDownValid
b TraverseDownFinished
TraverseDownValid:
add $t8, $t8, $t6   #move to element below us
lw $s6, ($t8)
...
TraverseRightUp:
sub $a1, $a1, 1      # the row moves up
add $a2, $a2, 1		#the column moves down
blt $a1, 0, TraverseRightUpFinished # if height is touched we are done
bge $a2, $t0, TraverseRightUpFinished # if width is touched we are done
TraverseRightUpValid:
sub $t8, $t8, $t6 	#goes up a row
add $t8, $t8, 4		# goes right one slot
lw $s6, ($t8)		# load slot to be compared
beq $s6, $s2, TraverseRightUpFinished
sw $s3, ($t8)		# stores 3 to mark unsafe 
b TraverseRightUp
...
TraverseRightDown:
add $a1, $a1, 1 # row increases downward
add $a2, $a2, 1	#column increases to right
...
TraverseLeftUp:
	sub $a1, $a1, 1	 # the row moves up
	sub $a2, $a2, 1	 # the column moves left
	blt $a1, 0, TraverseLeftUpFinished	# if height is touched we are done
	blt $a2, 0, TraverseLeftUpFinished	# if width is touched we are done
	TraverseLeftUpValid:
sub $t8, $t8, $t6	#goes up a row
sub $t8, $t8, 4		#goes left one slot
lw $s6, ($t8)	# load slot to be compared
...
TraverseLeftDown:
add $a1, $a1, 1 	# the row moves down
sub $a2, $a2, 1		# the column moves right
bge $a1, $t0, TraverseLeftDownFinished # if depth is touched we are done
blt $a2, 0, TraverseLeftDownFinished	# if width is touched we are done	
TraverseLeftDownValid:
sub $t8, $t8, $t6	# goes up 1 row
sub $t8, $t8, 4		# goes left one slot
lw $s6, ($t8)		#loads to compare
...
li $t1,1
PrintLoop:
li $v0, 1 # the code for printing an int
